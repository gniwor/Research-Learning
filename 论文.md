...
$$
因此，找到所有质数然后计算 $\pi(x)$ 所需时间复杂度至少为 $\mathcal O\left(\dfrac x{\log x}\right)$。
...
1985 年，Lagarias、Miller 和 Odlyzko 改编了 Meissel-Lehmer 方法并证明可以在 $\mathcal O\left(\dfrac {x^{2/3}}{\log x}\right)$ 时间、$\mathcal O\left(x^{1/3}\log^2x\log\log x\right)$ 空间内求出 $\pi(x)$。 他们计算到了 $\pi(4\cdot10^{16})$，同时还修正了 $\pi\left(10^{13}\right)$ 的值。

1987 年，Lagarias 和 Odlyzko 给出了一种完全不同的方法，基于关于黎曼函数 $\zeta$ 的某些积分变换的数值积分，对于每个 $\epsilon>0$，使用 $\mathcal O(x^{1/2+\epsilon})$ 时间和 $\mathcal O(x^{1/4+\epsilon})$ 空间计算 $\pi(x)$。尽管具有渐近复杂度的优势，但该算法从未实现。他们指出隐含常数可能很大，因此在 $x\leq10^{17}$ 时与 Meissel-Lehmer 方法相比没有竞争力。

这篇论文中，我们将给出一种基于 Meissel-Lehmer 方法的优化算法，可以在 $\mathcal O\left(\dfrac{x^{2/3}}{\log^2x}\right)$ 时间、$\mathcal O\left(x^{1/3}\log^3x\log\log x\right)$ 空间内计算 $\pi(x)$。
...
但是，为了达到我们的目标需要计算太多东西。因为 $y\geq x^{1/3}$，仅仅计算为 $3$ 个 $\leq y$ 质数的乘积的数，如果按照这个方法计算，会有至少 $\dfrac{x}{\log^3 x}$ 个项，没有办法我们对复杂度的需求。
...
更多的，当 $mp>x^{1/2}\ge y$ 时，有 $y\le pm$。因此我们有：
$$
...
我们将 $S_2$ 分成 $q>x/p^2$ 与 $q\le x/p^2$ 两部分：
$$
S_2=U+V
$$
其中：
...
更准确的说，我们首先将 $V_2$ 分成两个部分，将 $q\le \min\left(x/p^2,y\right)$ 这个复杂的条件简化：
$$
...
计算 $W_2$ 的时间复杂度为：
$$
O\left(\sum\limits_{x/y^2<p\le \sqrt{x/y}}{\pi\left(\sqrt{x/p}\right)} \right)=O\left(\dfrac{x^{3/4}}{y^{1/4}\log^2 x} \right)
$$
因此，计算 $W_3$ 的时间复杂度为：
$$
O\left(\sum\limits_{x/y^2<p\le \sqrt{x/y}}{\pi\left(\sqrt{x/p}\right)} \right)=O\left(\dfrac{x^{3/4}}{y^{1/4}\log^2 x} \right)
...
计算 $W_4$ 的时间复杂度为：
$$
O\left(\sum\limits_{\sqrt{x/y}<p\le x^{1/3}}{\pi\left(\sqrt{x/p}\right)} \right)=O\left(\dfrac{x^{2/3}}{\log^2 x} \right)
$$
...
### 8.3. 计算 $S_3$ 的复杂度
 ...
 The sieve. Owing to the necessity of quickly retrieving the values φ\left(u, b\right), we have to maintain a data structure such that each access costs O\left(log x\right) instead of O\left(1\right) in a normal sieve. Hence the cost is O\left( x y log x log log x\right). The sum. For each term in the sum we have to access the data structure mentioned above, doing O\left(log x\right) operations. It remains to count the terms in the sum. All leaves are of the form ±φ\left(x/mpb, b − 1\right) with m ≤ y and b<π\left(x1/4\right). Hence, the number of these leaves is O\left(yπ\left(x1/4\right)\right). The total cost of S3 is O x y log x log log x + yx1/4  . 
